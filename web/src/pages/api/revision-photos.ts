...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiOk | ApiErr>) {
    ...
    // GET /api/revision-photos?revisionId=...
    if (req.method === "GET") {
        const revisionId = String(req.query.revisionId || "").trim();
        if (!revisionId) return bad(res, "Missing revisionId");
    ...
    if (req.method === "POST") {
        ...
        if (!revision_id) return bad(res, "Missing revision_id");
        if (!base64) return bad(res, "Missing base64");

        // Check photo count first
        ...
        if ((photoCount ?? 0) >= 10) {
            return res.status(400).json({ ok: false, error: { code: "LIMIT_EXCEEDED", message: "Maximum 10 photos per revision" } });
        }
        ...
        const storage_path = `${revision_id}/${stamp}-${rand}-${safeName}`;
        ...
        const { data: inserted, error: insertErr } = await admin
            .from("revision_photos")
            .insert({ revision_id, uploaded_by: requester.id, url: publicUrl, storage_path })
            .select("*")
            .single();
    ...
    // DELETE /api/revision-photos?id=...
    if (req.method === "DELETE") {
        const id = String(req.query.id || "").trim();
        if (!id) return bad(res, "Missing id");

        const { data: photo } = await admin.from("revision_photos").select("*").eq("id", id).single();
        if (!photo) return res.status(404).json({ ok: false, error: { code: "NOT_FOUND", message: "Photo not found" } });

        if (photo.storage_path) {
            await admin.storage.from("revision-photos").remove([photo.storage_path]);
        }

        const { error } = await admin.from("revision_photos").delete().eq("id", id);
        if (error) return supaErr(res, error);
