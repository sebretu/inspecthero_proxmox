...
// 1. Core Logic Functions to test directly
export function computeItemProjectContribution(categoryWeight, itemWeight, itemProgress) {
  const fraction = (categoryWeight / 100) * (itemWeight / 100) * (itemProgress / 100);
  return Number((fraction * 100).toFixed(2));
}

export function toggleItemCompletion(currentProgress) {
  return currentProgress >= 100 ? 0 : 100;
}

export function getAllocationStatus(totalAllocation) {
...
export function calculateTreeProjectProgress(params) {
  const { projectId, currentSubproject = 'General', rawNodes } = params;
  const subprojectNodes = rawNodes.filter(
    (n) => (n.subproject || 'General').toLowerCase() === currentSubproject.toLowerCase()
  );
...
it('1. Leaf Node with manual 50% progress', () => {
  const rawNodes = [
    { id: 'cat-1', parentId: null, name: 'Planung', weight: 20 },
    { id: 'work-1', parentId: 'cat-1', name: 'Vorbereitung', weight: 100, progress: 50 },
  ];
...
it('3. Parent progress computed from children equals 37.5%', () => {
  const rawNodes = [
    { id: 'cat-1', parentId: null, name: 'Endmontage', weight: 10 },
    { id: 'work-1', parentId: 'cat-1', name: 'Mängelbeseitigung', weight: 10, progress: 0 },
